...
class HEALPixMap:
    ...
    def getMesh(self):
        ...
        for iN in range(1,self.nsides):
            ...
            # square pole caps
            if iN == 1:
                yield((iN,1,iN,2,iN,3))
                yield((iN,1,iN,3,iN,4))
                yield((iS,1,iS,2,iS,3))
                yield((iS,1,iS,3,iS,4))
            for g in range(4):
                for j in range(1,iN+1):
                    ...
                    yield((iN,j1,iN+1,j2,iN+1,j3))
                    yield((iS,j1,iS-1,j2,iS-1,j3))
                    # polar cap triangles point towards the equator
                    if j < iN:
                        yield((iN,j1,iN,j2+1,iN+1,j3))
                        yield((iS,j1,iS,j2+1,iS-1,j3))
                    # triangles filling in the rectangular holes in the polar caps
                    if j == 1:
                        # j1 = iN*g+1, j2 = (iN+1)*g+1, j3 = (iN+1)*g+2
                        yield((iN,j1,iN,j1-1,iN+1,j2-1))
                        yield((iS,j1,iS,j1-1,iS-1,j2-1))
                        yield((iN,j1,iN+1,j2,iN+1,j2-1))
                        yield((iS,j1,iS-1,j2,iS-1,j2-1))
        # equatorial belt
        for i in range(self.nsides,3*self.nsides):
            for j in range(1,4*self.nsides+1):
                dj = 1-(i+self.nsides)%2;
                yield((i,j,i,j+1,i+1,j+dj))
                yield((i,j+1,i+1,j+dj,i+1,j+dj+1))
    """
